...
void Rasterizer::DrawMesh(Framebuffer& fb, const Mesh& mesh, const Mat4f& mvp) {
    float width = static_cast<float>(fb.Width());
    float height = static_cast<float>(fb.Height());

    for (const auto& [fA, fB, fC]: mesh.f) {
        // MeshVertex -> ClipVertex
        ClipVertex cA = ProcessVertex(mesh.v[fA], mvp);
        ClipVertex cB = ProcessVertex(mesh.v[fB], mvp);
        ClipVertex cC = ProcessVertex(mesh.v[fC], mvp);

        // ClipVertex -> ScreenVertex
        ScreenVertex sA = ToScreen(cA, fb.Width(), fb.Height());
        ScreenVertex sB = ToScreen(cB, fb.Width(), fb.Height());
        ScreenVertex sC = ToScreen(cC, fb.Width(), fb.Height());
    }
}

ClipVertex Rasterizer::ProcessVertex(const MeshVertex& mv, const Mat4f& mvp) {
    return {
        .pos { mvp * Vec4f { mv.pos.x, mv.pos.y, mv.pos.z, 1.f } },
        .col { mv.col }
    };
}

ScreenVertex Rasterizer::ToScreen(const ClipVertex& cp, int w, int h) const {

}

};

// Rasterizer::Rasterizer(FramebufferLegacy2& fb)
//     : target_ {fb} 
// { }
